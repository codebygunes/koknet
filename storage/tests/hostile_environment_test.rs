...
fn test_hostile_environment_and_crdt_resolution() {
    ...
    let claimed_pub_key = VerifyingKey::from_bytes(
        &hex::decode(&crdt_v1.author_id).unwrap().try_into().unwrap()
    ).unwrap();
    ...
    assert!(!merged_v1, "Bob MUST reject V1 because he already has a newer timestamp for this ID.");
    ...
    assert_eq!(final_decrypted, b"Location Compromised! Move to Safehouse B");
